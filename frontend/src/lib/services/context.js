...
async function fetchRelevantKnowledge(userMessage, keywords) {
  // Optimization: Cache semantic reranking results to avoid redundant LLM calls.
  // Cache key includes normalized user message + sorted keywords.
  const normalizedMsg = (userMessage || '').trim().toLowerCase().replace(/\s+/g, ' ');
  const cacheKey = `knowledge_${normalizedMsg}_${keywords.slice().sort().join('|')}`;
