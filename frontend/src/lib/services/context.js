...
async function fetchRelevantKnowledge(userMessage, keywords) {
  ...
  const cacheKey = `knowledge:${userMessage.trim().toLowerCase()}:${sortedKeywords.join(',')}`;
  ...
  if (!ftsError) {
    const results = await rerankKnowledgeSemantically(userMessage, ftsRows || []);
  ...
  const results = await rerankKnowledgeSemantically(userMessage, fallbackRows || []);
  setCache(cacheKey, results, TTL_5MIN);
  return results;
}
