...
async function buildContext(userMessage = '') {
  ...
  if (userMessage && userMessage.length > 3) {
    // Build a plain-text FTS query from the message
    // Extract meaningful words (> 3 chars), skip common fillers
    ...
    if (keywords.length > 0) {
      const ftsQuery = keywords.join(' | '); // OR search across keywords
